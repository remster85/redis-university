    ...
    private Set<Site> findSitesByGeoWithCapacity(GeoQuery query) {
        ...
        GeoUnit radiusUnit = query.getRadiusUnit();
         ...
         try (Jedis jedis = jedisPool.getResource()) {
             ...
             //To see how this HashMap is used, study the for loop that comes after this
             //TODO. Hint: you effectively need to get a mapping of site IDs to their capacity.

            sites.forEach(site ->
                scores.put(site.getId(),
                pipeline.zscore(RedisSchema.getCapacityRankingKey(), Long.toString(site.getId())))
            );
